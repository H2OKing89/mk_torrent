...
| ------------------------------------------------------ | ---------------------------------------------------- | ---------: | -------------------------------------------------------------------- | ----------- | ------- | ------------------------ | ------------------------------------------- |
| `src/mk_torrent/api/trackers/base.py`                  | `src/mk_torrent/trackers/base.py`                    |       Move | One canonical tracker home under `trackers/*`.                       | TBA         | planned | TBA                      | Pre‑req for RED/MAM adapters                |
| `src/mk_torrent/api/trackers/red.py`                   | `src/mk_torrent/trackers/red/adapter.py`             |       Move | Unify tracker adapters; avoid duplicate packages.                    | TBA         | planned | TBA                      | Add shim at old path with warning           |
| `src/mk_torrent/api/trackers/mam.py`                   | `src/mk_torrent/trackers/mam/adapter.py`             |       Move | Same as above for MAM.                                               | TBA         | planned | TBA                      | Ensure imports updated in workflows         |
| `src/mk_torrent/trackers/upload_spec.py`               | `src/mk_torrent/core/upload/spec.py`                 | Move/Merge | Centralize shared upload types in `core` to reduce drift.            | TBA         | planned | N/A                      | Tracker deltas remain per‑tracker           |
| `src/mk_torrent/trackers/red/upload_spec.py`           | `src/mk_torrent/trackers/red/upload_spec.py`         |       Keep | Tracker‑specific fields extend `core.upload.spec`.                   | —           | planned | N/A                      | Update imports to core spec                 |
| `src/mk_torrent/features/templates.py`                 | `src/mk_torrent/core/metadata/templates/renderer.py` | Move/Merge | Consolidate templating with metadata pipeline.                       | TBA         | planned | TBA                      | Port helper functions; add tests            |
| `src/mk_torrent/api/qbittorrent.py`                    | `src/mk_torrent/integrations/qbittorrent.py`         |       Move | Single IO client location under `integrations/*`.                    | TBA         | planned | TBA                      | Ensure client is IO‑only; no business logic |
| `src/mk_torrent/core/upload/spec.py` (empty duplicate) | —                                                    |     Delete | Remove empty duplicate to eliminate confusion.                       | TBA         | planned | N/A                      | If both exist, keep the implemented one     |
| `src/mk_torrent/api/*` (general)                       | `src/mk_torrent/integrations/*`                      |       Move | Retire ambiguous `api/` in favor of `integrations/` for external IO. | TBA         | planned | TBA                      | Add shims per‑module as needed              |
