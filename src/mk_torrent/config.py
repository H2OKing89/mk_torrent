            ...
            for line in f:
                ...
                if "# SECURE_PASSKEY_STORED" in line:
                    ...
                    if SECURE_STORAGE_AVAILABLE:
                        ...
                        full_url = get_secure_tracker_url(
                            line
                        )  # Pass full line, not just base URL
                        if (
                            full_url and full_url != line.split("#")[0].strip()
                        ):  # Check if passkey was actually added
                            trackers.append(full_url)
                            console.print(
                                "[green]✓ Loaded tracker with secure passkey[/green]"
                            )
                        else:
                            console.print(
                                "[yellow]⚠️ No secure passkey found for tracker[/yellow]"
                            )
                    else:
                        console.print(
                            "[yellow]⚠️ Secure storage not available for tracker[/yellow]"
                        )
                else:
                    # Regular tracker URL
...
def store_qbittorrent_password(config: dict[str, Any], password: str):
    ...
    if not SECURE_STORAGE_AVAILABLE:
        ...
        console.print(
            "[yellow]⚠️ Storing password in plain text (secure storage not available)[/yellow]"
        )
        return
...
def get_default_config() -> dict[str, Any]:
    ...
    return {
        ...
        "docker_mappings": {"/mnt/cache": "/data", "/mnt/user": "/media"},
        "default_piece_size": "Auto",
        "default_private": True,
        "output_directory": str(Path.home() / "torrents"),
        "upload_queue_directory": str(
            Path.home() / "torrent_uploads"
        ),  # NEW: For pending uploads
...
def setup_wizard():
    ...
    console.print(
        Panel.fit(
            "[bold cyan]🔧 Torrent Creator Setup Wizard[/bold cyan]",
            border_style="cyan",
        )
    )
    ...
    if use_docker:
        ...
        for host_path, container_path in suggested_mappings:
            if Confirm.ask(
                f"Add mapping {host_path} → {container_path}?", default=True
            ):
    ...
    config["qbit_host"] = Prompt.ask(
        "qBittorrent host/IP", default=config.get("qbit_host", "localhost")
    )

    config["qbit_port"] = IntPrompt.ask(
        "qBittorrent port", default=config.get("qbit_port", 8080)
    )

    config["qbit_https"] = Confirm.ask(
        "Use HTTPS?", default=config.get("qbit_https", False)
    )

    config["qbit_username"] = Prompt.ask(
        "Username", default=config.get("qbit_username", "admin")
    )
    ...
    if SECURE_STORAGE_AVAILABLE:
        ...
        console.print(
            "[dim]Password will be stored securely using system keyring[/dim]"
        )
    ...
    if run_health_check(config):
        ...
        console.print(
            "[yellow]⚠️ Connection failed. Please check your settings.[/yellow]"
        )
    ...
    format_choice = Prompt.ask(
        "Select default format", choices=["1", "2", "3"], default="1"
    )
    ...
    console.print(
        "[dim]Setting a source tag helps identify torrents for safe cross-seeding[/dim]"
    )
    ...
    if config["default_source"]:
        console.print(
            f"[green]✓ Source tag '{config['default_source']}' will be added to torrents[/green]"
        )
        console.print(
            "[yellow]ℹ️ Remember to download .torrent files for cross-seeding on other trackers[/yellow]"
        )
    ...
    if categories:
        config["default_category"] = Prompt.ask(
            "Default category", choices=categories + [""], default=""
        )
    ...
    if tags:
        ...
        if Confirm.ask("Select default tags to apply to all torrents?", default=False):
            ...
            default_tags = [
                t.strip() for t in tags_input.split(",") if t.strip() in tags
            ]
...
def edit_config():
    ...
    console.print(
        Panel.fit("[bold cyan]⚙️ Configuration Editor[/bold cyan]", border_style="cyan")
    )
...
def edit_qbit_connection(config: dict[str, Any]):
    ...
    config["qbit_host"] = Prompt.ask(
        "Host/IP", default=config.get("qbit_host", "localhost")
    )
...
def edit_docker_settings(config: dict[str, Any]):
    ...
    config["docker_mode"] = Confirm.ask(
        "Use Docker mode?", default=config.get("docker_mode", False)
    )

    if config["docker_mode"]:
        config["docker_container"] = Prompt.ask(
            "Container name", default=config.get("docker_container", "qbittorrent")
        )
...
def edit_categories_tags(config: dict[str, Any]):
    ...
    if Confirm.ask("Edit categories?", default=False):
        while True:
            action = Prompt.ask(
                "Action", choices=["add", "remove", "done"], default="done"
            )
            if action == "done":
                break
            elif action == "add":
...
def edit_behavior(config: dict[str, Any]):
    ...
    config["default_private"] = Confirm.ask(
        "Make torrents private by default?", default=config.get("default_private", True)
    )
    ...
    if Confirm.ask("Change default format?", default=False):
        format_choice = Prompt.ask(
            "Default format", choices=["v1", "v2", "hybrid"], default=current_format
        )
        config["default_torrent_format"] = format_choice

        if format_choice == "v2":
            console.print(
                "[yellow]⚠️ Note: Many trackers don't support v2 torrents yet[/yellow]"
            )
    ...
    config["default_source"] = Prompt.ask(
        "Default source tag (empty for none)", default=config.get("default_source", "")
    )

    if config["default_source"]:
        console.print(
            f"[yellow]ℹ️ Torrents will be tagged with source '{config['default_source']}' for cross-seeding[/yellow]"
        )
