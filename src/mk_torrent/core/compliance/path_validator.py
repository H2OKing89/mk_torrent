...
class PathValidator:
    ...
    def __init__(self, tracker: str = 'default'):
        ...
        self.rule = self.TRACKER_RULES.get(self.tracker, 
                                           self.TRACKER_RULES['default'])
    ...
    def validate(self, path: Path) -> Tuple[bool, List[str]]:
        ...
        if len(path_str) > self.rule.max_length:
            issues.append(f"Path exceeds {self.rule.max_length} character limit "
            ...
            if file_path.is_file():
                ...
                if not valid:
                    invalid_files.append({
                        ...
                        'issues': issues,
                        'overage': len(full_path) - self.rule.max_length if len(full_path) > self.rule.max_length else 0
                    })
    ...
    def check_single_path(self, path_str: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'valid': valid,
            'issues': issues,
            'overage': len(path_str) - self.rule.max_length if len(path_str) > self.rule.max_length else 0,
            'tracker': self.tracker,
            'max_length': self.rule.max_length
        }
