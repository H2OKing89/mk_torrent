...
class TorrentCreator:
    def __init__(
        ...
        self.private = self.config.get(
            "default_private", True
        )  # Changed default to True for private trackers
        self.comment = ""
        ...
        self.auto_management = self.config.get(
            "auto_torrent_management", True
        )  # New setting
        self.torrent_format = self.config.get(
            "default_torrent_format", "v1"
        )  # Default to v1 for compatibility
        self.source = self.config.get(
            "default_source", ""
        )  # New: source field for cross-seeding
        self.optimize_alignment = self.config.get(
            "optimize_alignment", True
        )  # New: optimize alignment
        self.padded_file_size_limit = self.config.get(
            "padded_file_size_limit", -1
        )  # New: padding control
        ...
        if config:
            try:
                ...
                try:
                    ...
                    console.print(
                        "[dim]Using config password (secure storage not available)[/dim]"
                    )
                ...
                if api_version and api_version >= "2.10.4":
                    console.print(
                        f"[green]✓ Torrent Creator API supported (v{api_version})[/green]"
                    )
                else:
                    console.print(
                        f"[yellow]⚠️ API version {api_version} may not support torrent creation[/yellow]"
                    )

            except Exception as e:
                console.print(
                    f"[yellow]Warning: Could not connect to qBittorrent API: {e}[/yellow]"
                )
                self.client = None
    ...
    def load_default_trackers(self):
        ...
        try:
            ...
            if self.trackers:
                console.print(
                    f"[green]✓ Loaded {len(self.trackers)} tracker(s) with secure passkeys[/green]"
                )
            else:
                console.print(
                    "[yellow]No default trackers found. Add them manually.[/yellow]"
                )
        except ImportError:
        ...
        if self.config.get("auto_health_check", True):
            ...
            for name, info in checker.results.get("disk_space", {}).items():
                if not info.get("healthy", True):
                    console.print(
                        f"[yellow]⚠️ Low disk space on {name}: {info.get('free_gb', 0):.1f} GB free[/yellow]"
                    )
    ...
    def _convert_path_for_docker(self, path: Path) -> str:
        ...
        for host_path, container_path in docker_mappings.items():
            if path_str.startswith(host_path):
                ...
                console.print(
                    f"[dim]Docker path mapping: {path_str} -> {docker_path}[/dim]"
                )
                return docker_path

        # If no mapping found, return original path and warn
        console.print(
            f"[yellow]⚠️ No Docker mapping found for path: {path_str}[/yellow]"
        )
        ...
        try:
            ...
            if self.private and not self.trackers:
                ...
                if not self.trackers:
                    console.print(
                        "[red]✗ Private torrent requires at least one tracker[/red]"
                    )
                    return False
            ...
            if self.trackers:
                ...
                tracker_display = (
                    self.trackers[0][:50] + "..."
                    if len(self.trackers[0]) > 50
                    else self.trackers[0]
                )
            ...
            with Progress(
                ...
                while True:
                    ...
                    if state == TaskStatus.RUNNING:
                        ...
                        progress.update(
                            task_progress, description="Torrent created!", completed=100
                        )
                        break
                        ...
                        console.print(
                            f"[dim]Error: {getattr(status, 'message', 'Unknown error')}[/dim]"
                        )
                        task.delete()
                        return False
            ...
            if self.start_seeding:
                ...
                if not success:
                    console.print(
                        "[yellow]⚠️ Torrent created but not added to client[/yellow]"
                    )
    ...
    def _add_torrent_to_client(self, torrent_path: Path, content_path: Path) -> bool:
        ...
        try:
            ...
            console.print("[cyan]Adding torrent to qBittorrent with settings:[/cyan]")
            ...
            if response == "Ok.":
                ...
                if torrent_hash:
                    console.print(
                        f"[cyan]Found torrent hash: {torrent_hash[:16]}...[/cyan]"
                    )
                    ...
                    if self.category:
                        ...
                        self.client.torrents_set_category(
                            torrent_hashes=torrent_hash, category=self.category
                        )
                        time.sleep(1)
                    ...
                    if self.tags:
                        console.print(
                            f"[cyan]Setting tags: {', '.join(self.tags)}[/cyan]"
                        )
                        self.client.torrents_add_tags(
                            torrent_hashes=torrent_hash, tags=self.tags
                        )
                        time.sleep(1)
                    ...
                    if self.auto_management:
                        console.print(
                            "[cyan]Enabling automatic torrent management[/cyan]"
                        )
                        self.client.torrents_set_auto_management(
                            torrent_hashes=torrent_hash, enable=True
                        )
                        time.sleep(1)
                    ...
                    console.print(
                        "[green]✓ All torrent settings applied successfully[/green]"
                    )
                    return True
                else:
                    console.print(
                        "[yellow]⚠️ Torrent added but couldn't find hash for settings[/yellow]"
                    )
                    return True
            else:
    ...
    def create_torrent_for_upload(
        ...
        """
        console.print(
            f"[cyan]Creating torrent for upload to {tracker_config.get('name', 'tracker')}[/cyan]"
        )

        # Apply ALL settings from config automatically for RED uploads
        if (
            tracker_config.get("name") == "RED"
            or tracker_config.get("source_tag") == "RED"
        ):
            ...
            self.tags = self.config.get(
                "default_tags", ["redacted", "Uploaded:H2OKing"]
            )
            self.start_seeding = self.config.get("auto_start_seeding", True)
        ...
        if success and output_path.exists():
            ...
            try:
                ...
                console.print("[green]✓ Torrent verified:[/green]")
                console.print(
                    f"  Name: {torrent_data[b'info'][b'name'].decode('utf-8', errors='ignore')}"
                )
                console.print(
                    f"  Private: {torrent_data[b'info'].get(b'private', 0) == 1}"
                )
                console.print(
                    f"  Source: {torrent_data[b'info'].get(b'source', b'').decode('utf-8', errors='ignore')}"
                )
                console.print(
                    f"  Announce: {torrent_data.get(b'announce', b'').decode('utf-8', errors='ignore')[:50]}..."
                )
                ...
                console.print(
                    f"[yellow]Warning: Could not verify torrent: {e}[/yellow]"
                )
                return output_path
    ...
    def _find_torrent_by_path(self, source_path: str) -> Optional[str]:
        ...
        try:
            ...
            console.print(
                f"[dim]Searching through {len(torrents)} torrents for: {source_name}[/dim]"
            )
            ...
            for torrent in torrents:
                ...
                if torrent.name == source_name:
                    console.print(
                        f"[green]Found torrent by name match: {torrent.name}[/green]"
                    )
                    return torrent.hash
                ...
                if torrent_path == source_normalized:
                    console.print("[green]Found torrent by exact path match[/green]")
                    return torrent.hash
                ...
                if save_path == source_normalized:
                    console.print(
                        "[green]Found torrent by save_path + name match[/green]"
                    )
                    return torrent.hash
                ...
                if self.config.get("docker_mode", False):
                    ...
                    if docker_path and Path(docker_path) == torrent_path:
                        console.print(
                            "[green]Found torrent by Docker path mapping[/green]"
                        )
                        return torrent.hash
                ...
                if source_name in torrent.name or torrent.name in source_name:
                    ...
                    if (
                        hasattr(torrent, "added_on")
                        and time.time() - torrent.added_on < 300
                    ):
                        console.print(
                            f"[green]Found torrent by recent name similarity: {torrent.name}[/green]"
                        )
                        return torrent.hash

            console.print(
                f"[yellow]Could not find torrent for path: {source_normalized}[/yellow]"
            )
            return None
        except Exception as e:
    ...
    def _configure_torrent(self, torrent_hash: str):
        ...
        try:
            ...
            if self.client is not None:
                self.client.torrents_set_auto_management(
                    torrent_hashes=torrent_hash, enable=self.auto_management
                )
        ...
        try:
            ...
            if torrents:
                ...
                if self.category and torrent.category != self.category:
                    console.print(
                        f"[yellow]  ⚠️ Category mismatch: expected '{self.category}', got '{torrent.category}'[/yellow]"
                    )
                ...
                if expected_tags and not expected_tags.issubset(torrent_tags):
                    ...
                    console.print(
                        f"[yellow]  ⚠️ Missing tags: {', '.join(missing)}[/yellow]"
                    )

                # Verify auto management
                if torrent.auto_tmm != self.auto_management:
                    console.print(
                        f"[yellow]  ⚠️ Auto management: expected {self.auto_management}, got {torrent.auto_tmm}[/yellow]"
                    )
    ...
    def create_single(self):
        ...
        output_dir = Prompt.ask(
            "Output directory for .torrent file", default=default_output
        )
        ...
        if self.client:
            self.start_seeding = Confirm.ask(
            ...
            if self.source:
                ...
                console.print(
                    "[yellow]Remember to download the .torrent file for cross-seeding on other trackers[/yellow]"
                )

                # Offer to save a copy
                if Confirm.ask(
                    "Save a backup copy of the .torrent file?", default=True
                ):
                    backup_dir = Path(
                        self.config.get(
                            "backup_directory", Path.home() / "torrent_backups"
                        )
                    )
                    backup_dir.mkdir(parents=True, exist_ok=True)
                    backup_path = (
                        backup_dir / f"{source_path.name}_{self.source}.torrent"
                    )
                    console.print(f"[dim]Backup will be saved to: {backup_path}[/dim]")

            # Ask about automatic management
            self.auto_management = Confirm.ask(
            ...
            if self.category or Confirm.ask(
                "Set category?", default=bool(self.category)
            ):
        ...
        if success:
            console.print("[green]✅ Torrent created successfully![/green]")
            if self.start_seeding:
                console.print("[green]✅ Now seeding in qBittorrent[/green]")
    ...
    def batch_create_interactive(self):
        ...
        console.print(
            Panel.fit(
                "[bold cyan]📦 Batch Torrent Creation[/bold cyan]", border_style="cyan"
            )
        )
        ...
        if self.config.get("batch_health_check", True):
            ...
            if not run_quick_health_check(self.config):
                if not Confirm.ask(
                    "[yellow]Health check warnings detected. Continue?[/yellow]",
                    default=False,
                ):
                    return
        ...
        # Count items first
        total_items = len(
            [d for d in base_path.iterdir() if not d.name.startswith(".")]
        )
        ...
        for item in tqdm(items, desc="Creating torrents", unit="torrent"):
            ...
            if item.is_dir():
                ...
                if size > 10 * 1024**3:  # > 10GB
                    ...
                    output_dir_space = checker.results["disk_space"].get(
                        "output_directory", {}
                    )
                    if output_dir_space.get("free_gb", 0) < 1:
                        console.print(
                            "[red]❌ Insufficient disk space for torrent creation[/red]"
                        )
                        failed += 1
                        continue
    ...
    def _select_items_for_batch(self, base_path: Path) -> List[Path]:
        ...
        console.print(
            "[dim]Use SPACE to select/deselect, TAB to select all/none, ENTER to confirm[/dim]"
        )
    ...
    def _manual_select_items(self, items: List[tuple]) -> List[Path]:
        ...
        console.print(
            "[dim]Examples: '1-5' for range, '1,3,5' for specific items, 'all' for everything[/dim]"
        )
    ...
    def _configure_torrent_options(self):
        ...
        format_options = {"1": "v1", "2": "v2", "3": "hybrid"}
        ...
        if self.torrent_format == "v2":
            console.print(
                "[yellow]⚠️ Warning: Many trackers don't support v2 torrents yet![/yellow]"
            )
        ...
        console.print(
            "[dim]The source field helps identify torrents for cross-seeding[/dim]"
        )

        # Show common source tags from config
        common_sources = self.config.get(
            "common_sources", ["RED", "OPS", "GGn", "PTP", "BTN", "MAM"]
        )
        if common_sources:
        ...
        if Confirm.ask("Set source field?", default=bool(self.source)):
            self.source = Prompt.ask(
                "Source tag", default=self.source if self.source else ""
            )

            # Warn about cross-seeding implications
            if self.source:
                console.print(
                    f"[yellow]ℹ️ Source set to '{self.source}' - download the .torrent file for safe cross-seeding[/yellow]"
                )
        ...
        self.private = Confirm.ask(
            "Make private torrent?", default=self.config.get("default_private", True)
        )
                ...
                while True:
                    try:
                        ...
                        if 0 <= choice <= len(comment_templates):
                            break
                        console.print(
                            f"[red]Please select 0-{len(comment_templates)}[/red]"
                        )
                    except ValueError:
    ...
    def _configure_advanced_options(self):
        ...
        if Confirm.ask("Configure padding?", default=False):
            console.print(
                "[dim]Padding files help with alignment but increase torrent size[/dim]"
            )
            ...
            while True:
                try:
                    ...
                    if mb_limit >= -1:
                        self.padded_file_size_limit = (
                            mb_limit * 1024 * 1024 if mb_limit > 0 else mb_limit
                        )
                        break
    ...
    def _map_to_docker_path(self, host_path: Path) -> str:
        ...
        console.print(
            f"[yellow]Warning: No Docker mapping found for {host_path}[/yellow]"
        )
        return str_path
