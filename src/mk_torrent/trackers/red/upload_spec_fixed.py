...
class REDFormAdapter:
    ...
    def convert_to_form_data(self, spec: REDUploadSpec) -> dict[str, str]:
        ...
        if bitrate_value == "Other":
            ...
            # Based on old script pattern: use format like "64k VBR"
            form_data["other_bitrate"] = "64k VBR"  # Default for audiobooks
            form_data["vbr"] = "true"  # VBR format
        else:
        ...
        # Flags
        form_data["scene"] = "true" if spec.scene else "false"
        form_data["vanity_house"] = "true" if spec.vanity_house else "false"
