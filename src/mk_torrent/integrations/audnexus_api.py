    ...
    def _process_response(self, response) -> Optional[Dict[str, Any]]:
        ...
        try:
            if response.status_code == 200:
                data = response.json()
                logger.info(f"Successfully fetched Audnexus metadata")
                ...
                logger.warning(f"Audnexus API error: {error_data.get('message', 'Bad request')}")
                return None
            else:
    ...
    def _normalize_response(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if data.get('authors'):
                ...
                normalized['authors'] = [author.get('name') for author in authors if author.get('name')]
                normalized['artist'] = normalized['authors'][0] if normalized['authors'] else None
            
            # Extract and normalize narrators
            if data.get('narrators'):
                narrators = data['narrators']
                normalized['narrators_detailed'] = narrators
                normalized['narrators'] = [narrator.get('name') for narrator in narrators if narrator.get('name')]
                normalized['narrator'] = ', '.join(normalized['narrators']) if normalized['narrators'] else None
            ...
            if data.get('releaseDate'):
                try:
                    ...
                    normalized['release_date_formatted'] = release_date.strftime('%B %d, %Y')
                except:
                    pass
            ...
            if data.get('summary'):
                normalized['summary_cleaned'] = self._clean_html_content(data['summary'])
                normalized['description'] = normalized['summary_cleaned']
            
            if data.get('description') and not normalized.get('description'):
                normalized['description'] = self._clean_html_content(data['description'])
