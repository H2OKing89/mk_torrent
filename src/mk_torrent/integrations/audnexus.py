...
class AudnexusAPI:
        ...
        try:
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    logger.warning(f"Audnexus API error: {error_data.get('message', 'Unknown error')}")
                    return None
                else:
    ...
    def normalize_api_response(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if data.get('description'):
            metadata['description_cleaned'] = self._clean_html_content(data['description'])
        ...
        if data.get('seriesPrimary'):
            ...
            metadata['series'] = f"{series.get('name', '')} #{series.get('position', '')}"
