...
def upload_workflow(source_path: Path, tracker: str, config: Dict[str, Any], 
                   dry_run: bool = True, check_existing: bool = True) -> bool:
    """
    ...
    try:
        ...
        console.print(f"[green]✓ Metadata extracted ({metadata.get('content_type', 'unknown')} detected)[/green]")
        ...
        if check_existing:
            ...
            if search_params:
                ...
                if existing:
                    console.print(f"[yellow]⚠ Found {len(existing)} potentially matching torrents[/yellow]")
                    for i, torrent in enumerate(existing[:3], 1):  # Show first 3
                        title = torrent.get('groupName', torrent.get('title', 'Unknown'))
                        console.print(f"[dim]  {i}. {title}[/dim]")
                    
                    if not dry_run:
                        import typer
                        if not typer.confirm("Continue with upload despite existing torrents?"):
                            console.print("[yellow]Upload cancelled by user[/yellow]")
                            return False
                else:
                    console.print(f"[green]✓ No existing torrents found[/green]")
        ...
        if not validation['valid']:
            console.print(f"[red]✗ Metadata validation failed:[/red]")
        ...
        if dry_run:
            console.print(f"[yellow]DRY RUN: Would upload to {tracker.upper()}[/yellow]")
            console.print(f"[dim]  Source: {source_path}[/dim]")
            console.print(f"[dim]  Torrent: {torrent_path}[/dim]")
            console.print(f"[dim]  Content: {metadata.get('content_type', 'unknown')}[/dim]")
            return True
        else:
            # Actual upload
            ...
            if result.get('success'):
                console.print(f"[green]✓ Successfully uploaded to {tracker.upper()}![/green]")
                if result.get('url'):
...
def check_existing_torrents(tracker: str, metadata: Dict[str, Any], config: Dict[str, Any]) -> list:
    """
