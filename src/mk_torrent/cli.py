...
from .config import load_config, setup_wizard, show_config, edit_config
...
from .features.templates import save_template, list_templates
...
console = Console()
app = typer.Typer(
    help="Interactive Torrent Creator for qBittorrent", add_completion=False
)
...
def create(
    ...
    quick: bool = typer.Option(
        False, "--quick", "-q", help="Quick mode with minimal prompts"
    ),
):
    ...
    if batch and not run_quick_health_check(config):
        if not Confirm.ask(
            "[yellow]System health check warnings detected. Continue anyway?[/yellow]",
            default=False,
        ):
            raise typer.Exit(1)
...
def config(
    ...
    edit: bool = typer.Option(
        False, "--edit", "-e", help="Edit configuration interactively"
    ),
...
def health(
    comprehensive: bool = typer.Option(
        False, "--comprehensive", "-c", help="Run comprehensive health checks"
    ),
    monitor: bool = typer.Option(False, "--monitor", "-m", help="Monitor continuously"),
    duration: int = typer.Option(
        300, "--duration", "-d", help="Monitor duration in seconds"
    ),
):
...
def verify(
    ...
    for torrent in torrents:
        table.add_row(
            ...
            (
                torrent.tags[:27] + "..."
                if len(torrent.tags) > 30
                else torrent.tags or "[dim]none[/dim]"
            ),
...
def info():
    ...
    console.print(
        Panel.fit("[bold cyan]📊 System Information[/bold cyan]", border_style="cyan")
    )
    ...
    try:
        ...
        console.print("\n[cyan]qBittorrent Info:[/cyan]")
        console.print("  Connection: [red]✗ Not connected[/red]")
    ...
    if config.get("default_torrent_format") == "v2":
        ...
        console.print(
            "[yellow]Many private trackers don't support v2 torrents yet.[/yellow]"
        )
        console.print(
            "[yellow]Consider switching to v1 for better compatibility.[/yellow]"
        )
...
def metadata(
    ...
    show_description: bool = typer.Option(
        False, "--show-description", "-d", help="Show full RED description"
    ),
):
    ...
    try:
        ...
        if format == "table":
            ...
            if metadata.get("literatureType"):
                table.add_row(
                    "Literature Type", str(metadata["literatureType"]).title()
                )
            ...
            if metadata.get("channels"):
                channel_desc = (
                    ...
                    else f"{metadata['channels']} channels"
                )
            ...
            if metadata.get("chapters"):
                ...
                if isinstance(chapters, list) and chapters:
                    ...
                    if (
                        len(chapters) > 1
                        and chapters[-1].get("start_seconds") is not None
                    ):
                        ...
                        if isinstance(last_chapter_start, (int, float)):
                            ...
                            if total_hours > 0:
                                table.add_row(
                                    "Chapter Span", f"~{total_hours}h {total_minutes}m"
                                )
                            else:
            ...
            if verbose:
                ...
                summary = {
                    ...
                    "ready_for_upload": metadata.get("validation", {}).get(
                        "valid", False
                    ),
                }
...
def upload(
    ...
    tracker: str = typer.Option(
        "red", "--tracker", "-t", help="Tracker to upload to (red, ops)"
    ),
    dry_run: bool = typer.Option(
        True, "--dry-run/--no-dry-run", help="Perform dry run only"
    ),
    check_existing: bool = typer.Option(
        True, "--check-existing/--no-check", help="Check for existing torrents"
    ),
    ...
    if not config.get(api_key_field):
        ...
        console.print("[dim]Run 'python run.py config' to set up API credentials[/dim]")
        raise typer.Exit(1)
    ...
    if dry_run:
        ...
        if not Confirm.ask(
            f"Perform actual upload to {tracker.upper()}?", default=False
        ):
    ...
    try:
        success = upload_workflow(
            source_path, tracker, config, dry_run=dry_run, check_existing=check_existing
        )
        ...
        if not dry_run:
            console.print(
                "[dim]You may want to try --dry-run first to test the workflow[/dim]"
            )
        raise typer.Exit(1)
...
def batch(
    ...
    pattern: str = typer.Option(
        "", "--pattern", "-p", help="Filter items by pattern (e.g., '*.mkv')"
    ),
    recent: bool = typer.Option(
        False, "--recent", "-r", help="Sort by most recent first"
    ),
    skip_checks: bool = typer.Option(
        False, "--skip-checks", help="Skip connectivity checks"
    ),
):
    ...
    if not skip_checks:
        if not validate_qbittorrent_config(config):
            console.print(
                "[red]✗ qBittorrent connection failed. Run 'setup' first.[/red]"
            )
            raise typer.Exit(1)
    ...
    if path:
        ...
        base_path = (
            Path(Prompt.ask("Base directory", default=".")).expanduser().resolve()
        )
