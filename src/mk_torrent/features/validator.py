...
def validate_path(path_str: str) -> tuple[bool, list[str]]:
    ...
    console.print(
        Panel.fit(f"[bold cyan]🔍 Validating: {path}[/bold cyan]", border_style="cyan")
    )
    ...
    console.print("[green]✅ Path exists[/green]")

    # Check permissions
    if not path.stat().st_mode & 0o444:  # Read permission
        issues.append("No read permission")
    else:
        console.print("[green]✅ Read permission OK[/green]")
    ...
    if size == 0:
        ...
        warnings.append(
            f"Very large torrent ({format_size(size)}), may take time to create"
        )
    ...
    if path.is_dir():
        ...
        if problematic:
            ...
            console.print(
                "[yellow]⚠️ Files with special characters may cause issues[/yellow]"
            )
    ...
    try:
        ...
        with open(torrent_path, "rb") as f:
            ...
            if len(content) < 100:  # Arbitrary minimum size
                console.print(
                    "[red]Torrent file appears to be invalid (too small)[/red]"
                )
                return False
