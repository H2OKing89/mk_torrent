...
class FormatDetector:
    ...
    def analyze(self, source_files: List[Path]) -> Dict[str, Any]:
        ...
        if not MUTAGEN_AVAILABLE:
            console.print(
                "[yellow]⚠️ Mutagen not available - limited format detection[/yellow]"
            )
    ...
    def _extract_format_info(self, audio: Any, file_path: Path) -> Dict[str, Any]:
        ...
        if isinstance(audio, FLAC):
            ...
            bitrate = getattr(info, "bitrate", None)
            format_data.update(
                {"lossless": False, "encoding": f"AAC {bitrate}k" if bitrate else "AAC"}
            )
            # Special handling for M4B audiobooks
            if file_ext == ".m4b":
                format_data["encoding"] = (
                    f"M4B Audiobook ({bitrate}k)" if bitrate else "M4B Audiobook"
                )
...
class AudnexusAPI:
    ...
    def _normalize_audnexus_data(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if data.get("summary"):
                metadata["summary_cleaned"] = self._clean_html_summary(
                    data.get("summary", "")
                )
                metadata["summary"] = metadata[
                    "summary_cleaned"
                ]  # Use cleaned version as primary
            ...
            if authors:
                ...
                metadata["authors_detailed"] = (
                    authors  # Keep full author objects with ASINs
                )
                metadata["artist"] = (
            ...
            if metadata.get("releaseDate"):
                try:
                    ...
                    metadata["release_date_formatted"] = release_date.strftime(
                        "%B %d, %Y"
                    )
...
class MetadataEngine:
    ...
    def process_metadata(
        ...
        with Progress(
            ...
            if metadata.get("tags"):
                ...
                metadata["tags"] = self.tag_normalizer.normalize(
                    metadata.get("tags", [])
                )
            else:
                progress.advance(task)
        ...
        console.print("[green]✓ Metadata processing complete[/green]")
        if validation_result["errors"]:
            console.print(
                f"[yellow]⚠️ {len(validation_result['errors'])} validation warnings[/yellow]"
            )
    ...
    def _extract_file_metadata(self, source_files: List[Path]) -> Dict[str, Any]:
        ...
        if not MUTAGEN_AVAILABLE:
            console.print(
                "[yellow]⚠️ Mutagen not available - using basic extraction[/yellow]"
            )
            ...
            if audio:
                ...
                metadata.update(
                    {
                        ...
                        "comment": self._extract_tag(
                            audio, "genre"
                        ),  # genre fallback for comment
                        "files": audio_files,
                ...
                if primary_file.suffix.lower() == ".m4b":
                    ...
                    if chapters:
                        ...
                        console.print(
                            f"[green]📚 Extracted {len(chapters)} chapters from M4B file[/green]"
                        )
                ...
                if metadata.get("genre"):
                    if isinstance(metadata["genre"], str):
                        metadata["tags"] = [
                            g.strip() for g in metadata["genre"].split(",")
                        ]
                    else:
    ...
    def _extract_chapters(self, audio: Any, file_path: Path) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if chapters:
                ...
                console.print("[yellow]📖 No chapters found in M4B file[/yellow]")
    ...
    def _parse_mp4_chapters(self, file_path: Path) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.debug(
                f"Could not parse chapters from {file_path} using external tools"
            )
            ...
            if "extra" in menu_track:
                ...
                if chapters:
                    for i in range(len(chapters) - 1):
                        ...
                        chapters[i]["duration"] = (
                            chapters[i]["end_time"] - chapters[i]["start_time"]
                        )
    ...
    def _extract_tag(self, audio: Any, tag_type: str) -> Optional[str]:
        ...
        tag_names = tag_mappings.get(tag_type, {}).get(
            file_format, tag_mappings.get(tag_type, {}).get("default", [])
        )
            ...
            if api_metadata:
                ...
                for key, value in api_metadata.items():
                    if value is not None and value != "":
                        ...
                        if (
                            key not in existing_metadata
                            or not existing_metadata.get(key)
                            or key
                            in [
                                "summary",
                                "description",
                ...
                logger.info(
                    f"Successfully enriched metadata with audnexus API data for ASIN: {asin}"
                )
                ...
                logger.warning(
                    f"No metadata returned from audnexus API for ASIN: {asin}"
                )
    ...
    def _build_enhanced_description(self, api_metadata: Dict[str, Any]) -> str:
        ...
        if api_metadata.get("releaseDate"):
            try:
                release_date = datetime.fromisoformat(
                    api_metadata["releaseDate"].replace("Z", "+00:00")
                )
                metadata_info.append(
                    f"**Release Date:** {release_date.strftime('%B %d, %Y')}"
                )
    ...
    def validate_metadata(self, metadata: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if metadata.get("format") not in valid_formats:
            validation["warnings"].append(
                f"Format {metadata.get('format')} may not be optimal for RED"
            )
        ...
        if metadata.get("format") in ["MP3", "AAC"]:
            ...
            if bitrate < 320000:  # 320 kbps
                validation["warnings"].append(
                    f"Bitrate {bitrate//1000}kbps is below recommended 320kbps for lossy format"
                )
