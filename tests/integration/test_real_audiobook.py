...
@pytest.fixture
def audiobook_info():
    """Fixture to provide audiobook information for tests"""
    # Analyze both audiobook locations and return the best one
    ...
    # If no audiobook found, skip tests that need it
    pytest.skip("No test audiobook files found")


@pytest.fixture
def sample_metadata():
    """Fixture providing sample metadata for tests that don't need real extraction"""
    return {
        ...
        "media": "WEB",
        "type": "audiobook",
        "folder_name": "How a Realist Hero Rebuilt the Kingdom - vol_03 (2023) (Dojyomaru) {ASIN.B0C8ZW5N6Y} [H2OKing]",
    }
...
def test_path_validation():
    ...
    test_cases = [
        ...
        (
            "Test samples path",
            str(TEST_PATHS["samples"]),
            False,
        ),  # Full path too long
        (
            "Seedvault path",
            str(TEST_PATHS["seedvault"]),
            False,
        ),  # Full path too long
...
def test_metadata_extraction(audiobook_info):
    ...
    engine.setup_default_processors()  # Initialize default processors
    ...
    # Basic validation that we got some metadata
    assert enhanced_metadata is not None
    assert "format" in enhanced_metadata
    assert enhanced_metadata["format"] == "M4B"


def test_red_validation(sample_metadata):
    ...
    folder_name = sample_metadata.get("folder_name", "")
    if folder_name:
        is_compliant = red_api.check_path_compliance(folder_name)
        console.print(
            f"📏 Folder name compliance: {'✅ PASS' if is_compliant else '❌ FAIL'}"
        )
        console.print(
            f"   '{folder_name}' ({len(folder_name)}/{red_api.config.max_path_length} chars)"
        )

    # Test release type detection
    release_type = red_api._detect_release_type(sample_metadata)
    expected_type = red_api.RELEASE_TYPES["SOUNDTRACK"]  # Audiobooks use SOUNDTRACK
    console.print(
        f"🎭 Release type: {release_type} ({'✅ Correct' if release_type == expected_type else '❌ Incorrect'})"
    )

    assert validation is not None
    # Don't assert validation['valid'] as it might fail due to missing fields


def test_upload_preparation(sample_metadata):
...
def test_dry_run_upload(sample_metadata):
    """Test dry run upload"""
    ...
    result = red_api.upload_torrent(dummy_torrent, sample_metadata, dry_run=True)
...
def main():
    ...
    if test_info:
        ...
        if success and metadata:
            ...
            results.update(
                {"red_validation": False, "upload_preparation": False, "dry_run": False}
            )
    else:
    ...
    if all_passed:
        ...
        console.print(
            "• Test CLI: python scripts/cli/red_upload_cli.py /path/to/audiobook --dry-run"
        )
