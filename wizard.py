...
def run_wizard():
    ...
    console.print(Panel.fit("[bold cyan]🧙 Torrent Creation Wizard[/bold cyan]", border_style="cyan"))
...
def wizard_single_torrent():
    ...
    if not creator.client:
        console.print("[red]❌ qBittorrent API not connected. Please run setup first.[/red]")
        return
    ...
    if preset == "public":
        ...
        creator.torrent_format = config.get("default_torrent_format", "v1")  # Use configured format, default v1
    else:
        # Custom configuration
        creator.load_default_trackers()
        creator.private = Confirm.ask("Make private?", default=config.get("default_private", True))
        creator.auto_management = Confirm.ask("Enable automatic management?", 
        ...
        if format_choice == "v2":
            console.print("[yellow]⚠️ Warning: Many trackers don't support v2 torrents[/yellow]")
    ...
    if creator.create_torrent_via_api(path, output_path):
        ...
        if creator.start_seeding:
            console.print("[green]✅ Torrent added to qBittorrent and seeding started[/green]")
            ...
            console.print(f"  Auto Management: {'Enabled' if creator.auto_management else 'Disabled'}")
    else:
...
def wizard_music_collection():
    ...
    for item in base_dir.iterdir():
        if item.is_dir():
            ...
            music_files = list(item.glob("*.mp3")) + list(item.glob("*.flac")) + list(item.glob("*.m4a"))
            if music_files:
    ...
    if album_folders:
        ...
        for folder in album_folders[:10]:  # Show first 10
            tracks = list(folder.glob("*.mp3")) + list(folder.glob("*.flac")) + list(folder.glob("*.m4a"))
...
def show_tutorial():
    ...
    console.print(Panel.fit("[bold cyan]📚 Torrent Creation Tutorial[/bold cyan]", border_style="cyan"))
...
def create_tv_torrents(base_dir: Path, folders: List[Path], strategy: str):
    ...
    creator.torrent_format = config.get("default_torrent_format", "v1")  # Use v1 for compatibility
    ...
    if strategy == "per-season":
        ...
        if creator.create_torrent_via_api(base_dir, output_path):
            console.print(f"[green]✓ Created series torrent: {output_path.name}[/green]")
        else:
            console.print(f"[red]✗ Failed to create series torrent[/red]")
...
def wizard_multiple_folders():
    ...
    # Count items
    all_items = [d for d in base_dir.iterdir() if d.is_dir() and not d.name.startswith('.')]
    ...
    if strategy == "first-n":
        ...
        while True:
            try:
                ...
                if 1 <= n <= len(all_items):
                    break
                console.print(f"[red]Please enter a number between 1 and {len(all_items)}[/red]")
            except ValueError:
        ...
        while True:
            try:
                n = IntPrompt.ask(f"Select top N largest folders (1-{len(items_with_size)})", default=5)
                if 1 <= n <= len(items_with_size):
                    break
                console.print(f"[red]Please enter a number between 1 and {len(items_with_size)}[/red]")
            except ValueError:
                console.print("[red]Please enter a valid number[/red]")
...
def wizard_recreate_torrents():
    ...
    console.print("[yellow]This feature helps you recreate torrents from existing data[/yellow]")
    ...
    if not torrent_files:
        ...
        console.print("[dim]Place the original .torrent files in the same directory as the data[/dim]")
        return
