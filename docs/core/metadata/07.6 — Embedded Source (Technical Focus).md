...
- **Precise duration**: Exact seconds from audio stream analysis
- **Audio codec details**: Format, profile, encoding parameters
- **Quality metrics**: Bitrate, sample rate, channel configuration
- **File system info**: Exact byte size and format detection
- **Cover art detection**: Embedded artwork presence and dimensions

### Chapter Information
- **Chapter timing**: Precise start/end times from container metadata
- **Chapter count**: Accurate detection of embedded chapter markers
- **Chapter titles**: Basic title extraction when available
- **Format support**: MP4/M4B chapters, FLAC cue points, etc.

### Multi-Backend Architecture
- **ffprobe (preferred)**: Fast, accurate, comprehensive format support
- **mutagen (fallback)**: Broader compatibility, Python-native
- **basic file info (minimal)**: File size and extension when tools unavailable
