...
class CrossSeedManager:
    ...
    def prepare_for_cross_seed(self, 
                               original_torrent: Path, 
                               new_tracker: str,
                               new_source: str) -> Path:
        """
    ...
    def verify_cross_seed_compatibility(self, 
                                       torrent1: Path, 
                                       torrent2: Path) -> Tuple[bool, str]:
        """
        ...
        try:
            ...
            if b'files' in data1[b'info']:
                # Multi-file torrent
                files1 = sorted([(f[b'path'], f[b'length']) for f in data1[b'info'][b'files']])
                files2 = sorted([(f[b'path'], f[b'length']) for f in data2[b'info'][b'files']])
                if files1 != files2:
