...
from mk_torrent.core.compliance.path_fixer import (
    PathFixer,
    ComplianceConfig,
    ComplianceLog,
)
...
def should_include_directory(
    ...
    if exclude_reasons:
        return {"include": False, "reason": "excluded", "details": exclude_reasons}
    ...
    if include_reasons:
        return {"include": True, "reason": "included", "details": include_reasons}
    else:
        return {
            "include": False,
            "reason": "not_matched",
...
def detect_tracker_intent(dir_path: Path) -> Dict[str, Any]:
    ...
    return {
        ...
        "safe_for_red": not detected_trackers
        or "red" in detected_trackers
        or "h2oking" in dir_name.lower(),
    }
...
def handle_single_audiobook(args):
    ...
    if not args.force:
        ...
        if not args.quiet and tracker_intent["detected_trackers"]:
            console.print(
                f"[cyan]🔍 Detected tracker indicators: {', '.join(tracker_intent['detected_trackers'])}[/cyan]"
            )

        if not tracker_intent["safe_for_red"] and not args.quiet:
            console.print(
                f"[yellow]⚠️  Warning: This directory appears to be intended for {tracker_intent['primary_intent'].upper()}[/yellow]"
            )
            console.print(
                "[yellow]   Renaming might break compatibility with that tracker.[/yellow]"
            )
            ...
            if args.apply:
                ...
                if response not in ["y", "yes"]:
                    console.print(
                        "[cyan]📌 Cancelled. Use --force to override safety checks.[/cyan]"
                    )
                    return 0
    ...
    if not args.quiet:
        ...
        console.print(
            f"🎵 Found {len(audio_files)} audio files, {len(files)} total files"
        )
    ...
    config = ComplianceConfig(
        max_full_path=args.max_length, dry_run=not args.apply, apply=args.apply
    )
    ...
    new_folder, new_files, log_entries = fixer.fix_path(
        str(audiobook_path),
        files,
        apply_changes=args.apply,  # pass ABS path
    )

    # Verify fixes worked
    new_folder_path = audiobook_path.parent / new_folder
    final_analysis = analyze_path_compliance(
        new_folder_path, new_files, args.max_length
    )
...
def display_compliance_table(
    ...
    if violation_count > 0:
        ...
        if hard_linked_files > 0:
            console.print(
                f"[yellow]🔗 {hard_linked_files} files have hard links - renaming will preserve them[/yellow]"
            )
    else:
...
def handle_batch_scan(args):
    ...
    for dir_path in all_audiobook_dirs:
        filter_result = should_include_directory(
            dir_path, include_patterns, exclude_patterns
        )
        ...
        if filter_result["include"]:
            ...
            if args.verbose:
                console.print(
                    f"[dim]Excluded: {dir_path.name} - {filter_result['reason']}[/dim]"
                )
    ...
    if not args.quiet:
        ...
        console.print(
            f"  Compliant: {compliant_dirs} ({compliant_dirs/total_dirs*100:.1f}%)"
        )
        console.print(
            f"  Need fixes: {issues_found} ({issues_found/total_dirs*100:.1f}%)"
        )
        if issues_found > 0:
            console.print(
                f"  Auto-fixable: {fixable_issues} ({fixable_issues/issues_found*100:.1f}% of issues)"
            )
            if risky_changes > 0:
                console.print(
                    f"  [yellow]⚠️  Risky changes: {risky_changes} (may affect other trackers)[/yellow]"
                )
    ...
    if args.generate_script and risky_changes > 0 and not args.force:
        ...
        console.print(
            "[yellow]   Review the generated script before running to avoid breaking compatibility.[/yellow]"
        )
        console.print(
            "[yellow]   Use --force to include risky changes or --exclude to filter them out.[/yellow]"
        )

    # Generate batch script if requested
    if args.generate_script:
        generate_batch_script(
            results, args.generate_script, args.force, args.max_length
        )
    ...
    if args.apply_batch:
        ...
        for r in results:
            if not r["compliant"] and r["fix_estimate"].get("fixable", False):
                if (not r.get("safe_for_red", True)) and (not args.force):
                    console.print(
                        f"[yellow]⚠️ Skipping risky dir (use --force): {Path(r['path']).name}[/yellow]"
                    )
                    skipped_count += 1
                    continue
                target = Path(r["path"])
                ...
                fixer = PathFixer(
                    tracker="red",
                    config=ComplianceConfig(
                        max_full_path=args.max_length, dry_run=False, apply=True
                    ),
                )
                try:
                    console.print(f"  🔧 Fixing: {target.name}")
                    new_folder, new_files, _ = fixer.fix_path(
                        str(target), files, apply_changes=True
                    )
                    applied_count += 1
        ...
        if skipped_count > 0:
            console.print(
                f"[green]✅ Batch apply completed! Fixed {applied_count} directories, skipped {skipped_count} risky.[/green]"
            )
        else:
            console.print(
                f"[green]✅ Batch apply completed! Fixed {applied_count} directories.[/green]"
            )
...
def estimate_fix_potential(
    ...
    try:
        ...
        # Use ABS path here ↓
        new_folder, new_files, log_entries = fixer.fix_path(
            str(dir_path), files, apply_changes=False
        )

        # Analyze using the *proposed* folder name only; dir need not exist
        final_analysis = analyze_path_compliance(
            Path(new_folder), new_files, max_length
        )
...
def generate_batch_script(
    ...
    script_lines = [
        ...
        "# Generated by mk_torrent path compliance tool",
        "",
    ]
    ...
    for result in results:
        if not result["compliant"] and result["fix_estimate"].get("fixable", False):
            ...
            if is_safe or force_risky:
                if not is_safe:
                    script_lines.append("# WARNING: May affect other trackers!")
                    script_lines.append(
                        f'# Detected: {result.get("tracker_intent", {}).get("primary_intent", "unknown")}'
                    )
                ...
                script_lines.append(
                    f'{py} "{me}" --max-length {max_length} "{path}" --apply{force_flag}'
                )
                script_lines.append("")
                ...
                script_lines.append(
                    f'# Detected tracker: {result.get("tracker_intent", {}).get("primary_intent", "unknown")}'
                )
                script_lines.append(
                    f'# Uncomment next line to include: {py} "{me}" "{path}" --apply --force'
                )
                script_lines.append("")

    total_count = safe_count + risky_count
    script_lines.append(
        f'echo "Batch fix completed: {total_count} directories processed ({safe_count} safe, {risky_count} risky)"'
    )
    ...
    if risky_count > 0:
        if force_risky:
            console.print(
                f"[dim]   {risky_count} risky directories included (--force used)[/dim]"
            )
        else:
            console.print(
                f"[dim]   {risky_count} risky directories commented out (use --force to include)[/dim]"
            )
...
Examples:
  ...
  python path_compliance.py "/path/to/audiobooks" --batch --export-json report.json
    ...
    parser.add_argument(
        "path", help="Path to audiobook folder or parent directory for batch mode"
    )
    parser.add_argument(
        "--max-length",
        type=int,
    ...
    parser.add_argument(
        "--apply", action="store_true", help="Apply changes (default: dry-run only)"
    )
    parser.add_argument(
        "--verbose", "-v", action="store_true", help="Show detailed change preview"
    )
    ...
    # Mode selection
    parser.add_argument(
        "--batch", action="store_true", help="Batch mode: scan multiple audiobooks"
    )
    ...
    parser.add_argument(
        "--generate-script", help="Generate bash script to fix all issues"
    )
