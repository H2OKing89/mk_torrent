...
def create_torrent_manually(source_path: Path, announce_url: str, output_path: Path) -> bool:
    ...
    try:
        ...
        if source_path.is_file():
            ...
            console.print("[yellow]⚠️  Directory hashing not implemented in this demo[/yellow]")
...
def test_complete_red_workflow():
    ...
    try:
        ...
        console.print(f"✅ RED API initialized")
        ...
        title = folder_name.split(' - vol')[0] if ' - vol' in folder_name else folder_name.split('(')[0].strip()
        ...
        important_fields = ['submit', 'type', 'artists[]', 'groupname', 'year', 'releasetype', 'format', 'tags']
        ...
        summary_table.add_row("File Size", f"{m4b_file.stat().st_size / (1024*1024):.1f} MB")
        ...
        if torrent_path.exists():
            ...
            console.print("[dim]You can use this torrent file for actual RED upload[/dim]")
        
        console.print("\n[bold green]🎉 Complete RED workflow test successful![/bold green]")
        console.print("[green]Your audiobook is fully prepared and ready for RED upload![/green]")
...
def main():
    ...
    if success:
        ...
        console.print("[green]Your RED implementation is working correctly with real data.[/green]")
