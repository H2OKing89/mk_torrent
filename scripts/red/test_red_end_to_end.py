...
project_root = Path(__file__).parent.parent.parent  # Go up to project root.parent  # Go up to project root
...
def test_red_api_creation():
    ...
    try:
        ...
        red_direct = RedactedAPI(api_key='test_key_456')
        console.print("✅ Created RED API directly")
        
        # Test configuration
        config = red_api.get_tracker_config()
        console.print(f"✅ Config: {config.name}, Source: {config.source_tag}, Max Path: {config.max_path_length}")
...
def test_red_metadata_validation():
    ...
    try:
        ...
        console.print(f"✅ Errors: {len(validation['errors'])}, Warnings: {len(validation['warnings'])}")
...
def test_red_release_type_detection():
    ...
    try:
        ...
        for metadata, expected_type in test_cases:
            ...
            console.print(f"✅ {metadata.get('album', metadata.get('type'))}: {expected_type} (ID: {detected})")
...
def test_red_path_compliance():
    ...
    try:
        ...
        test_paths = [
            ...
            ('Very long path that definitely exceeds REDs strict 150 character limit and should fail validation completely' * 2, False)
        ]
        
        for path, should_be_valid in test_paths:
            is_valid = red_api.check_path_compliance(path)
            status = "✅" if is_valid == should_be_valid else "❌"
            console.print(f"{status} Path length {len(path)}: {'Valid' if is_valid else 'Invalid'}")
...
def main():
    ...
    # Run all tests
    test_functions = [
        ('api_creation', test_red_api_creation),
        ('metadata_validation', test_red_metadata_validation),
        ('release_type_detection', test_red_release_type_detection),
        ('upload_data_preparation', test_red_upload_data_preparation),
        ('path_compliance', test_red_path_compliance),
        ('dry_run_upload', test_red_dry_run_upload),
        ('search_functionality', test_red_search_functionality),
    ]
    
    for test_name, test_func in test_functions:
        if test_name == 'api_creation':
            success, red_api = test_func()
