...
def create_real_torrent():
    ...
    try:
        ...
        sample_path = Path("/mnt/cache/scripts/mk_torrent/tests/samples/audiobook/How a Realist Hero Rebuilt the Kingdom - vol_03 (2023) (Dojyomaru) {ASIN.B0C8ZW5N6Y} [H2OKing]")
        ...
        # Prepare torrent creation parameters
        torrent_params = {
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            console.print(f"📏 Total size: {torrent_info['total_size'] / (1024*1024):.1f} MB")
...
def test_complete_metadata_workflow():
    ...
    try:
        ...
        combined_metadata = {
            ...
            # From M4B metadata
            'narrator': audio.tags.get('©wrt', ['Unknown'])[0] if audio.tags else 'Unknown',
            'publisher': audio.tags.get('cprt', ['Unknown'])[0] if audio.tags else 'Unknown',
            'genre': audio.tags.get('©gen', ['Unknown'])[0] if audio.tags else 'Unknown',
            'duration': f"{int(audio.info.length)} seconds" if audio.info else 'Unknown',
        ...
        upload_data = red_api.prepare_upload_data(combined_metadata, dummy_torrent)
        ...
        important_fields = ['title', 'artists', 'year', 'format', 'narrator', 'publisher', 'genre', 'duration']
...
def test_red_dry_run_with_real_data():
    ...
    try:
        ...
        console.print(f"  • Duration: {int(metadata['duration'].split()[0]) // 3600}h {(int(metadata['duration'].split()[0]) % 3600) // 60}m")
        ...
        console.print(f"  • Release Type: {upload_data['releasetype']} (Soundtrack for audiobooks)")
...
def test_integration_readiness():
    ...
    try:
        ...
        path_validator = PathValidator('red')
        ...
        try:
            invalid_tracker = get_tracker_api('nonexistent')
