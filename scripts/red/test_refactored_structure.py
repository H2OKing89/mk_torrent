...
def test_tracker_api():
    ...
    try:
        ...
        from mk_torrent.api.trackers import list_available_trackers
        ...
        console.print(
            f"✅ RED config: {config.name}, max_length={config.max_path_length}"
        )
...
def test_compliance_system():
    ...
    try:
        ...
        from mk_torrent.core.compliance.path_fixer import PathFixer
        ...
        console.print(
            f"✅ Tracker comparison for long path: {len(comparison)} trackers"
        )

        # Test path fixer
        fixer = PathFixer("red")
        console.print(
            f"✅ Path fixer created for RED (max: {fixer.config.max_full_path})"
        )
