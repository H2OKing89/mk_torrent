...
AUDIOBOOK_PATH = Path("/mnt/cache/scripts/mk_torrent/tests/samples/audiobook/How a Realist Hero Rebuilt the Kingdom - vol_03 (2023) (Dojyomaru) {ASIN.B0C8ZW5N6Y} [H2OKing]")
...
def extract_audiobook_info():
    ...
    try:
        ...
        if not m4b_file:
            console.print(f"[red]❌ No M4B file found[/red]")
            return None
...
def test_red_api_integration(metadata):
    ...
    try:
        ...
        console.print(f"✅ Validation result: {'PASS' if validation['valid'] else 'FAIL'}")
        ...
        release_name = [k for k, v in red_api.RELEASE_TYPES.items() if v == release_type][0]
        ...
        console.print(f"✅ Path compliance: {'PASS' if is_compliant else 'FAIL'} (length: {path_length}/150)")
        
        # Test 4: Upload data preparation
        console.print("[cyan]Testing upload data preparation...[/cyan]")
        upload_data = red_api.prepare_upload_data(red_metadata, Path("/tmp/test.torrent"))
        
        required_fields = ['submit', 'type', 'artists[]', 'groupname', 'year', 'releasetype']
        all_present = all(field in upload_data for field in required_fields)
        console.print(f"✅ Upload data preparation: {'PASS' if all_present else 'FAIL'}")
        ...
        dry_run_result = red_api.upload_torrent(Path("/tmp/test.torrent"), red_metadata, dry_run=True)
...
def test_red_workflow_simulation():
    ...
    try:
        ...
        if not validation['valid']:
            console.print(f"[red]❌ Metadata validation failed: {validation['errors']}[/red]")
            return False
        ...
        upload_data = red_api.prepare_upload_data(red_metadata, torrent_path)
        ...
        console.print("[green]Ready for real RED upload with actual API credentials![/green]")
...
def display_red_readiness_report(metadata):
    ...
    is_audiobook = 'audiobook' in AUDIOBOOK_PATH.name.lower() or metadata.get('format') == 'M4B'
    table.add_row(
        "Content Type",
    ...
    if all(critical_checks):
        ...
        console.print("[green]All critical requirements met. Add real API credentials to proceed.[/green]")
    else:
...
def main():
    ...
    if red_success and workflow_success:
        ...
        console.print("[yellow]Some components need attention before real upload.[/yellow]")
        return 1
