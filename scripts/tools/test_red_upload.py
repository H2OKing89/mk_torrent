...
This script supports both DRY RUN mode (preview only) and REAL UPLOAD mode.
...
AVAILABLE_FILES = {
    ...
    "evil_lord": {
        ...
        "audiobook_path": "/mnt/cache/scripts/mk_torrent/tests/samples/audiobook/Im the Evil Lord of an Intergalactic Empire - vol_03 (2025) (Yomu Mishima) {ASIN.B0FPXPYTKF} [H2OKing]",
        "torrent_file": "Im the Evil Lord of an Intergalactic Empire - vol_03 (2025) (Yomu Mishima) {ASIN.B0FPXPYTKF} [H2OKing].torrent",
    },
    "otome_games": {
        "name": "The World of Otome Games Is Tough for Mobs - vol_05",
        "audiobook_path": "/mnt/cache/scripts/mk_torrent/tests/samples/audiobook/The World of Otome Games Is Tough for Mobs - vol_05 (2025) (Yomu Mishima) {ASIN.B0FPXQH971} [H2OKing]",
        "torrent_file": "The World of Otome Games Is Tough for Mobs - vol_05 (2025) (Yomu Mishima) {ASIN.B0FPXQH971} [H2OKing].torrent",
    },
}

# Default to "otome_games" since it's the newest test file
DEFAULT_FILE = "otome_games"
...
def display_upload_preview(
    ...
    # Form Data Flags (Debug)
    adapter = REDFormAdapter()
    form_data = adapter.convert_to_form_data(upload_spec)

    flags_table = Table(title="🚩 Upload Flags")
    flags_table.add_column("Flag", style="cyan")
    flags_table.add_column("Value", style="white")

    flags_table.add_row("Scene Release", form_data.get("scene", "NOT_SET"))
    flags_table.add_row("Vanity House", form_data.get("vanity_house", "NOT_SET"))
    ...
    if upload_spec.description:
        # Show full description for testing our BBCode fixes
        console.print(Syntax(upload_spec.description, "bbcode", word_wrap=True))
        console.print(
...
def main():
    ...
    try:
        ...
        # Use the longer summary content for description if available (FIX for summary field issue)
        ...
        # Calculate real bitrate string from extracted metadata (NO FALLBACKS!)
        if metadata.bitrate and metadata.bitrate_mode:
            bitrate_kbps = metadata.bitrate // 1000
            real_other_bitrate = f"{bitrate_kbps}k {metadata.bitrate_mode}"
        else:
            console.print(
                "❌ [bold red]ERROR: No real bitrate data extracted from file![/bold red]"
            )
            console.print("   This indicates a problem with metadata extraction.")
            return

        # Validate that we have real data (NO FALLBACKS!)
        if not red_fields.get("title"):
            console.print(
                "❌ [bold red]ERROR: No real title extracted from file![/bold red]"
            )
            return

        if not red_fields.get("artists[]"):
            console.print(
                "❌ [bold red]ERROR: No real artist extracted from file![/bold red]"
            )
            return

        if not red_fields.get("year"):
            console.print(
                "❌ [bold red]ERROR: No real year extracted from file![/bold red]"
            )
            return

        upload_spec = REDUploadSpec(
            title=red_fields["title"],  # Real extracted title (no fallback)
            artists=[
                Artist(
                    name=red_fields["artists[]"][
                        0
                    ],  # Real extracted artist (no fallback)
                    type=ArtistType.MAIN,
                )
            ],
            year=int(red_fields["year"]),  # Real extracted year (no fallback)
            category="Audiobooks",
            format=AudioFormat.AAC,  # Based on our real file analysis
            bitrate=AudioBitrate.OTHER,  # Using "Other" with other_bitrate field
            other_bitrate=real_other_bitrate,  # Real calculated bitrate string
            release_type=ReleaseType.ALBUM,
            media="WEB",
            tags=tags_list,  # Real extracted tags
            description=red_fields.get("album_desc", ""),  # Real generated description
        )
