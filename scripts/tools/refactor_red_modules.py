...
def show_refactor_plan():
    ...
    mappings = [
        (
            "api/red_integration.py",
            "workflows/tracker_integration.py",
            "Generic tracker integration workflow",
        ),
        ("features/red_uploader.py", "api/trackers/red.py", "RED-specific API client"),
        (
            "features/metadata_engine.py",
            "core/metadata/engine.py",
            "Generic metadata processing",
        ),
        (
            "utils/red_path_compliance.py",
            "core/compliance/path_fixer.py",
            "Path compliance fixing logic",
        ),
        ("utils/red_compliance_rename.py", "[DEPRECATED]", "Replaced by path_fixer.py"),
        ("utils/red_api_parser.py", "utils/api_parser.py", "Generic API doc parser"),
    ]
...
def show_current_status():
    ...
    console.print(
        "1. Move metadata_engine.py → core/metadata/engine.py (replace generic version)"
    )
...
def integrate_existing_metadata():
    ...
    _new_metadata = Path("src/mk_torrent/core/metadata/engine.py")

    if old_metadata.exists():
        console.print(f"[green]Found existing metadata engine: {old_metadata}[/green]")

        if Confirm.ask(
            "Replace generic engine with existing implementation?", default=True
        ):
            # Would backup and replace the metadata engine
            console.print(
                "[yellow]Implementation needed: Merge existing metadata_engine.py[/yellow]"
            )
            return True
