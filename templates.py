...
def load_templates() -> List[Dict[str, Any]]:
    ...
    for template_file in templates_dir.glob("*.json"):
        try:
            ...
            console.print(f"[yellow]Warning: Could not load template {template_file.name}: {e}[/yellow]")
...
def apply_template_cli(template_name: str, source_path: Path) -> bool:
    ...
    template = None
    for t in templates:
        if t.get('name') == template_name or t.get('filename') == f"{template_name}.json":
            template = t
            break
    ...
    console.print(f"[cyan]Applying template: {template.get('name', template_name)}[/cyan]")
...
def edit_template(templates: Dict[str, Any]):
    ...
    template["private"] = Confirm.ask("Private?", default=template.get("private", False))
    template["piece_size"] = Prompt.ask("Piece size", default=template.get("piece_size", "Auto"))
