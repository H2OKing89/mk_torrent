...
class QBittorrentAPI:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if response_text == "Ok.":
                    ...
                    console.print(f"[yellow]Unexpected login response: {response_text}[/yellow]")
            ...
            console.print("[yellow]Make sure qBittorrent Web UI is enabled and accessible[/yellow]")
            return False
    ...
    def sync_categories_and_tags(self, config: Dict[str, Any]) -> None:
        ...
        new_tags = [tag for tag in config.get("common_tags", []) if tag not in existing_tags]
        if new_tags:
...
def run_health_check(config: Dict[str, Any]) -> bool:
    ...
    table.add_row(
        ...
        f"{api.base_url}" + (f" (API v{results['api_version']})" if results["api_version"] else "")
    )
    ...
    if results["errors"]:
        ...
        console.print("4. Check 'Bypass authentication for clients on localhost' if accessing locally")
        console.print("5. Or add your IP to 'Bypass authentication for clients in whitelisted IP subnets'")
...
def test_docker_connectivity(container_name: str) -> bool:
    ...
    try:
        ...
        if container_name in result.stdout:
            console.print(f"[green]✅ Docker container '{container_name}' is running[/green]")
            ...
            if test_result.returncode == 0:
                console.print(f"[green]✅ Can execute commands in container[/green]")
                return True
            else:
                console.print(f"[red]❌ Cannot execute commands in container[/red]")
                return False
        else:
